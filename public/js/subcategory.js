document.addEventListener("DOMContentLoaded", function () {
    const table = document.getElementById("subcategoriesTable");

    table.addEventListener("click", function (event) {
        if (event.target.closest(".btn-delete")) {
            const row = event.target.closest("tr"); // Obtiene la fila del botón clicado
            const subcategoryId = row.querySelector("td").innerText.trim(); // Toma el ID de la subcategoría

            if (!confirm("¿Estás seguro de que deseas eliminar esta subcategoría?")) {
                return;
            }

            // Realizar la solicitud DELETE
            fetch(`/subcategorias/delete/${subcategoryId}`, {
                method: "DELETE",
                headers: {
                    "X-CSRF-TOKEN": document.querySelector('meta[name="csrf-token"]').getAttribute("content"),
                },
            })
                .then(response => {
                    if (!response.ok) {
                        throw new Error("Error al eliminar la subcategoría.");
                    }
                    return response.json();
                })
                .then(data => {
                    if (data.success) {
                        alert("Subcategoría eliminada con éxito.");
                        row.remove(); // Elimina la fila de la tabla
                    } else {
                        alert("Hubo un problema al eliminar la subcategoría.");
                    }
                })
                .catch(error => {
                    console.error("Error:", error);
                    alert("Ocurrió un error al eliminar la subcategoría.");
                });
        }
    });
});
